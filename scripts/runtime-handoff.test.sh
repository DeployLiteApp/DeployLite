...
DATABASE_URL="postgres://deploylite:strong#=password@postgres:5432/deploylite"
DEPLOYLITE_SECRET_KEY='0123456789abcdef'
DEPLOYLITE_ACME_EMAIL="operator@acme.co" # inline comment
ENV
...
test_success_order_and_redaction() {
  ...
  [[ "$(printf '%s\n' "$log" | awk '/config --no-interpolate/{a=NR} /config --environment/{b=NR} /--profile bootstrap up -d --wait/{u++; if (u == 1) c=NR; if (u == 2) e=NR} /run --rm --no-deps migrate/{d=NR} END { print (a < b && b < c && c < d && d < e) }')" == 1 ]] || return 1
}

test_invalid_acme_email_is_rejected_without_leak() {
  bash -c 'source "$1"; ! validate_acme_email "not-an-email" "app.example.com"' _ "$ROOT_DIR/scripts/runtime-handoff.sh"
}
test_same_domain_and_subdomain_acme_emails_are_accepted() {
  bash -c 'source "$1"; validate_acme_email "contact@deploylite.com" "deploylite.com" && validate_acme_email "ops@sub.deploylite.com" "deploylite.com" && validate_acme_email "ops@deploylite.com" "sub.deploylite.com"' _ "$ROOT_DIR/scripts/runtime-handoff.sh"
}
test_url_like_and_host_without_at_acme_values_are_rejected() {
  bash -c 'source "$1"; for email in "deploylite.com" "https://deploylite.com" "https://deploylite.com/"; do if validate_acme_email "$email" "deploylite.com"; then exit 1; fi; done' _ "$ROOT_DIR/scripts/runtime-handoff.sh"
}
test_reserved_acme_domains_are_case_insensitive_and_real_tlds_are_allowed() {
  bash -c 'source "$1"; for email in admin@your-domain.tld ADMIN@YOUR-DOMAIN.TLD operator@acme.test operator@ACME.TEST user@example.com user@service.invalid user@service.localhost; do validate_acme_email "$email" app.example.com && exit 1; done; validate_acme_email "operator@acme.co" app.example.com' _ "$ROOT_DIR/scripts/runtime-handoff.sh"
}
