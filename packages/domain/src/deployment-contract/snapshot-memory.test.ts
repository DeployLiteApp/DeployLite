...
describe("snapshot store", () => it("is immutable and hash keyed", () => { const store = new InMemorySnapshotStore(); const saved = store.save(snapshot); expect(store.get(snapshot.hash)).toEqual(saved); expect(store.save(snapshot)).toEqual(saved); expect(() => store.save({ ...snapshot, projectId: "changed" })).toThrow("immutable"); (saved as { deploymentId: string }).deploymentId = "changed"; expect(store.get(snapshot.hash)?.deploymentId).toBe("d"); }));
