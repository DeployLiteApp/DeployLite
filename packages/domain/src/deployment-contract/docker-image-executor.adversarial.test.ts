...
describe("DockerImageExecutor adversarial boundaries", () => {
  ...
  ])("rejects tampered or unsafe %s without transport", (_name, changes) => { const transport = new FakeDockerImageTransport(); const { protocol, executor } = setup(transport); const base = snapshot(changes as Partial<DeploymentSnapshotV1>); const candidate = _name === "canonical JSON" ? { ...base, canonicalJson: "{}" } : _name === "canonical bytes" ? { ...base, canonicalBytes: new TextEncoder().encode("{}") } : base; expect(() => executor.execute(input(protocol, { snapshot: candidate }))).toThrow(); expect(transport.calls).toEqual([]); });

  it("rejects an untrusted host and malformed digest before protocol effects", () => { const transport = new FakeDockerImageTransport(); const { protocol, executor } = setup(transport); const bad = snapshot({ source: { ...snapshot().source, image: { ...snapshot().source.image, registryHost: "evil.example.com", reference: `evil.example.com/team/app@${digest}` } } } as Partial<DeploymentSnapshotV1>); expect(() => executor.execute(input(protocol, { snapshot: bad }))).toThrow("trusted"); expect(transport.calls).toEqual([]); });

  it("rejects expired and fenced leases before transport", () => { let now = 1000; const transport = new FakeDockerImageTransport(); const expiredSetup = setup(transport, { now: () => now }); const expired = expiredSetup.protocol.claimLease("dep-1"); now = 1010; expect(() => expiredSetup.executor.execute({ snapshot: snapshot(), commandId: "expired", lease: expired })).toThrow(LeaseExpiredError); const fencedSetup = setup(transport); const old = fencedSetup.protocol.claimLease("dep-1"); const current = fencedSetup.protocol.claimLease("dep-1"); expect(() => fencedSetup.executor.execute({ snapshot: snapshot(), commandId: "stale", lease: old })).toThrow(FenceError); expect(current.fence).toBeGreaterThan(old.fence); expect(transport.calls).toEqual([]); });

  it("rejects a replay with a changed payload without invoking transport again", () => { const transport = new FakeDockerImageTransport(); const { protocol, executor } = setup(transport); executor.execute(input(protocol)); const changed = snapshot({ runtimePort: 3001 }); expect(() => executor.execute({ snapshot: changed, commandId: "command-1", lease: protocol.claimLease("dep-1") })).toThrow(ReplayConflictError); expect(transport.calls).toEqual(["start", "health", "promote"]); });

  it("records rollback failure as a failed terminal outcome", () => { const prior = { deploymentId: "dep-1", effectiveImage: `registry.example.com/team/old@${digest}`, runtimePort: 3000, health: "passed" as const, terminalStatus: "succeeded" as const, rollback: { target: null, result: "not-required" as const }, proven: true as const }; const transport = new FakeDockerImageTransport({ promote: new Error("promote failed"), restore: new Error("restore failed") }); const { protocol, executor } = setup(transport); const result = executor.execute(input(protocol, { priorProvenReceipt: prior })); expect(result).toMatchObject({ terminalStatus: "failed", rollback: { target: prior.effectiveImage, result: "not-available" } }); expect(protocol.getAck("dep-1")).toMatchObject({ kind: "terminal-ack", status: "failed" }); expect(transport.calls).toEqual(["start", "health", "promote", "restore", "discard"]); });

  it("emits a canceled terminal ACK without any transport effect", () => { const transport = new FakeDockerImageTransport(); const { protocol, executor } = setup(transport); const controller = new AbortController(); controller.abort(); const result = executor.execute(input(protocol, { signal: controller.signal })); expect(result.terminalStatus).toBe("canceled"); expect(protocol.getAck("dep-1")).toMatchObject({ kind: "terminal-ack", status: "canceled" }); expect(transport.calls).toEqual([]); });
});
