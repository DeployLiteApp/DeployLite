...
const integrationFlag = "DEPLOYLITE_DB_INTEGRATION";
...
describe("database PostgreSQL integration test gating", () => {
  it("fails clearly when enabled without DATABASE_URL", () => {
    const result = runIntegrationTest(true);

    expect(result.error).toBeUndefined();
    expect(result.status).toBe(1);
    expect(`${result.stdout}${result.stderr}`).toContain("DATABASE_URL must be set when DEPLOYLITE_DB_INTEGRATION=1.");
  }, 30_000);
