...
import { Folder, Gauge, LayoutDashboard, Menu, Rocket, Server, Settings2, X } from "lucide-react";
import { usePathname } from "next/navigation";
import { useEffect, useRef, useState, type ReactNode, type RefObject } from "react";
import { LogoutButton } from "@/app/auth-controls";
import { getAuthApiBaseUrl } from "@/lib/auth-boundary";

type AppShellProps = { email: string; children: ReactNode };
...
function Brand() {
  return <Link href="/dashboard" className="flex items-center gap-2.5 font-semibold tracking-tight md:gap-3" aria-label="DeployLite overview">
    ...
    <span className="translate-y-[3.25px] text-[17px] md:translate-y-0 md:text-lg">DeployLite</span>
  </Link>;
}
...
export function AppShell({ email, children }: AppShellProps) {
  ...
  return <div className="min-h-screen bg-[#fafafa] text-foreground dark:bg-[#18181b]">
    ...
    <header className="relative z-10 flex h-[70px] items-center justify-between border-b border-[#e4e4e7] bg-white px-5 dark:border-[#27272a] dark:bg-[#18181b] md:ml-[244px] md:h-[72px] md:px-8">
      <div className="md:hidden"><Brand /></div>
      <span className="hidden text-sm font-medium text-muted-foreground md:block">{navItems.find((item) => isActive(pathname, item.href))?.label ?? "Overview"}</span>
    ...
    <main className="min-h-[calc(100vh-70px)] bg-[#fafafa] px-5 pb-24 pt-[34px] dark:bg-[#18181b] md:ml-[244px] md:min-h-[calc(100vh-72px)] md:px-8 md:py-8">{children}</main>
