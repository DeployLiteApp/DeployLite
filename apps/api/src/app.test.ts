...
describe("DeployLite API scaffold", () => {
  ...
  it("denies read-only callers because audit events are operator/admin scoped", async () => {
    ...
    it("redeploys a persisted project-owned snapshot with confirmation, lineage, and replay", async () => {
      ...
      const { app } = await authFixture({ state: { deployments, snapshots: { saveSnapshot: async () => undefined, findByHash: async (hash) => snapshots.get(hash) ?? null }, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(app); const url = `/api/v1/deployments/${sourceDeployment.id}/redeploy`; const headers = { cookie, "x-control-idempotency-key": "redeploy-once" };
      ...
      const completed = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(completed.statusCode, completed.body).toBe(200); expect(completed.json().data.deployment).toMatchObject({ sourceDeploymentId: sourceDeployment.id, snapshotHash: snapshot.hash, status: "queued" });
