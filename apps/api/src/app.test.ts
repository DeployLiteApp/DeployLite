...
describe("DeployLite API scaffold", () => {
  ...
  it("denies read-only callers because audit events are operator/admin scoped", async () => {
    ...
    it("denies redeploy without authorization and rejects an ineligible snapshot", async () => {
      const deployments = new InMemoryDeploymentRepository(); const source = { id: "dep-redeploy-denied", projectId: "project_mock_1", agentId: "agent-1", status: "succeeded" as const, commitSha: "abcdef1", startedAt: "2026-01-01T00:00:00.000Z", finishedAt: null }; await deployments.save(source);
      const snapshot = createDeploymentSnapshot({ deploymentId: source.id, projectId: source.projectId, agentId: source.agentId, commitSha: source.commitSha, source: createSourceIntent({ sourceMode: "image", requestedReference: "registry.example.com/app:stable" }, { policyVersion: "p1", trustedHosts: ["registry.example.com"], allowTags: true, allowDigests: true }), configRevision: "c1", runtimeRevision: "r1", runtimePort: 3000, secretRefs: [], policyVersion: "p1", schemaVersion: 1 }, { sha256: () => "a".repeat(64) });
      const snapshots = { saveSnapshot: async () => undefined, findByHash: async () => snapshot };
      const denied = await authFixture({ state: { deployments, snapshots } }); const deniedCookie = await loginCookie(denied.app); const url = `/api/v1/deployments/${source.id}/redeploy`; const headers = { cookie: deniedCookie, "x-control-idempotency-key": "redeploy-denied" }; expect((await denied.app.inject({ method: "POST", url, headers, payload: { snapshotHash: snapshot.hash } })).statusCode).toBe(403);
      const eligibleDeployments = new InMemoryDeploymentRepository(); await eligibleDeployments.save(source); const eligible = await authFixture({ state: { deployments: eligibleDeployments, snapshots, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(eligible.app); const response = await eligible.app.inject({ method: "POST", url, headers: { cookie, "x-control-idempotency-key": "redeploy-ineligible" }, payload: { snapshotHash: snapshot.hash } }); expect(response.statusCode).toBe(409); expect(response.json().error.code).toBe("REDEPLOY_SNAPSHOT_INELIGIBLE");
    });

    it("rejects a mutated confirmation and admits one same-key execution", async () => {
      const deployments = new InMemoryDeploymentRepository(); const source = { id: "dep-redeploy-concurrent", projectId: "project_mock_1", agentId: "agent-1", status: "succeeded" as const, commitSha: "abcdef1", startedAt: "2026-01-01T00:00:00.000Z", finishedAt: null }; await deployments.save(source); const snapshot = createDeploymentSnapshot({ deploymentId: source.id, projectId: source.projectId, agentId: source.agentId, commitSha: source.commitSha, source: createSourceIntent({ sourceMode: "image", requestedReference: `registry.example.com/app@sha256:${"a".repeat(64)}` }, { policyVersion: "p1", trustedHosts: ["registry.example.com"], allowTags: false, allowDigests: true }), configRevision: "c1", runtimeRevision: "r1", runtimePort: 3000, secretRefs: [], policyVersion: "p1", schemaVersion: 1 }, { sha256: () => "c".repeat(64) }); const snapshots = { saveSnapshot: async () => undefined, findByHash: async () => snapshot };
      const fixture = await authFixture({ state: { deployments, snapshots, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(fixture.app); const url = `/api/v1/deployments/${source.id}/redeploy`; const headers = { cookie, "x-control-idempotency-key": "redeploy-concurrent" }; const pending = await fixture.app.inject({ method: "POST", url, headers, payload: { snapshotHash: snapshot.hash } }); const confirmationId = pending.json().data.confirmationId; expect((await fixture.app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": "mutated" }, payload: { snapshotHash: snapshot.hash } })).statusCode).toBe(409);
       const concurrentHeaders = { cookie, "x-control-idempotency-key": "redeploy-concurrent-2" }; const concurrentPending = await fixture.app.inject({ method: "POST", url, headers: concurrentHeaders, payload: { snapshotHash: snapshot.hash } }); const concurrentConfirmation = concurrentPending.json().data.confirmationId; const results = await Promise.all([fixture.app.inject({ method: "POST", url, headers: { ...concurrentHeaders, "x-control-confirmation-id": concurrentConfirmation }, payload: { snapshotHash: snapshot.hash } }), fixture.app.inject({ method: "POST", url, headers: { ...concurrentHeaders, "x-control-confirmation-id": concurrentConfirmation }, payload: { snapshotHash: snapshot.hash } })]); expect(results.some((result) => result.statusCode === 503)).toBe(true); expect(results.some((result) => result.statusCode === 200)).toBe(true); expect((await deployments.list()).filter((deployment) => deployment.sourceDeploymentId === source.id)).toHaveLength(1);
    });

    it("redeploys a persisted project-owned snapshot with confirmation, lineage, and replay", async () => {
      const deployments = new InMemoryDeploymentRepository(); const snapshots = new Map<string, ReturnType<typeof createDeploymentSnapshot>>();
      const sourceDeployment = { id: "dep-redeploy-source", projectId: "project_mock_1", agentId: "agent_mock_1", status: "succeeded" as const, commitSha: "abcdef1", startedAt: "2026-01-01T00:00:00.000Z", finishedAt: "2026-01-01T00:01:00.000Z" }; await deployments.save(sourceDeployment);
      const snapshot = createDeploymentSnapshot({ deploymentId: sourceDeployment.id, projectId: sourceDeployment.projectId, agentId: sourceDeployment.agentId, commitSha: sourceDeployment.commitSha, source: createSourceIntent({ sourceMode: "image", requestedReference: `registry.example.com/team/app@sha256:${"a".repeat(64)}` }, { policyVersion: "p1", trustedHosts: ["registry.example.com"], allowTags: false, allowDigests: true }), configRevision: "c1", runtimeRevision: "r1", runtimePort: 3000, secretRefs: [], policyVersion: "p1", schemaVersion: 1 }, { sha256: (bytes) => Buffer.from(bytes).toString("hex").slice(0, 64).padEnd(64, "0") }); snapshots.set(snapshot.hash, snapshot);
       const { app } = await authFixture({ state: { deployments, snapshots: { saveSnapshot: async () => undefined, findByHash: async (hash) => snapshots.get(hash) ?? null }, deploymentDispatcher: { available: () => true, dispatch: async () => { throw new Error("agent unavailable"); } }, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(app); const url = `/api/v1/deployments/${sourceDeployment.id}/redeploy`; const headers = { cookie, "x-control-idempotency-key": "redeploy-once" };
      const pending = await app.inject({ method: "POST", url, headers, payload: { snapshotHash: snapshot.hash } }); expect(pending.statusCode).toBe(202); const confirmationId = pending.json().data.confirmationId;
       const completed = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(completed.statusCode, completed.body).toBe(502); expect(completed.json().error.code).toBe("REDEPLOY_DISPATCH_FAILED"); expect((await deployments.list()).find((deployment) => deployment.sourceDeploymentId === sourceDeployment.id)?.status).toBe("failed");
      await deployments.remove(sourceDeployment.id); const replay = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(replay.statusCode).toBe(200); expect(replay.json().data.idempotent).toBe(true); expect(await deployments.findById(sourceDeployment.id)).toBeNull();
      const conflict = await app.inject({ method: "POST", url, headers, payload: { snapshotHash: "b".repeat(64) } }); expect(conflict.statusCode).toBe(409); expect(conflict.json().error.code).toBe("IDEMPOTENCY_CONFLICT");
    });

    it("returns pending for a concurrent owner and replays terminal evidence after source deletion", async () => {
      const deployments = new InMemoryDeploymentRepository(); const source = { id: "dep-redeploy-owner", projectId: "project_mock_1", agentId: "agent_mock_1", status: "succeeded" as const, commitSha: "abcdef1", startedAt: "2026-01-01T00:00:00.000Z", finishedAt: null }; await deployments.save(source);
      const snapshot = createDeploymentSnapshot({ deploymentId: source.id, projectId: source.projectId, agentId: source.agentId, commitSha: source.commitSha, source: createSourceIntent({ sourceMode: "image", requestedReference: `registry.example.com/team/app@sha256:${"a".repeat(64)}` }, { policyVersion: "p1", trustedHosts: ["registry.example.com"], allowTags: false, allowDigests: true }), configRevision: "c1", runtimeRevision: "r1", runtimePort: 3000, secretRefs: [], policyVersion: "p1", schemaVersion: 1 }, { sha256: () => "a".repeat(64) });
       let release!: () => void; let dispatches = 0; const gate = new Promise<void>((resolve) => { release = resolve; }); const dispatcher: DeploymentDispatcher = { available: () => true, async dispatch(_snapshot, _commandId, context) { dispatches++; await gate; return { deploymentId: context!.executionDeploymentId!, projectId: _snapshot.projectId, sourceDeploymentId: _snapshot.deploymentId, snapshotHash: _snapshot.hash, correlationId: context!.correlationId, candidateId: `${context!.executionDeploymentId}:candidate:run`, effectiveImage: `registry.example.com/team/app@sha256:${"a".repeat(64)}`, runtimePort: 3000, health: "passed", terminalStatus: "succeeded", rollback: { target: null, result: "not-required" }, proven: true }; } };
      const snapshots = { saveSnapshot: async () => undefined, findByHash: async () => snapshot }; const { app } = await authFixture({ state: { deployments, snapshots, deploymentDispatcher: dispatcher, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(app); const url = `/api/v1/deployments/${source.id}/redeploy`; const headers = { cookie, "x-control-idempotency-key": "redeploy-owner" }; const pending = await app.inject({ method: "POST", url, headers, payload: { snapshotHash: snapshot.hash } }); const confirmationId = pending.json().data.confirmationId;
       const first = app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); await new Promise((resolve) => setTimeout(resolve, 0)); const second = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(second.statusCode).toBe(202); expect(second.json().data.pending).toBe(true); expect(dispatches).toBe(1); release(); const firstResponse = await first; expect(firstResponse.statusCode, firstResponse.body).toBe(200); await deployments.remove(source.id); const replay = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(replay.statusCode).toBe(200); expect(replay.json().data.idempotent).toBe(true);
     });
     it("rejects a custom dispatcher receipt before terminal persistence", async () => {
       const deployments = new InMemoryDeploymentRepository(); const source = { id: "dep-redeploy-mismatch", projectId: "project_mock_1", agentId: "agent_mock_1", status: "succeeded" as const, commitSha: "abcdef1", startedAt: "2026-01-01T00:00:00.000Z", finishedAt: null }; await deployments.save(source); const snapshot = createDeploymentSnapshot({ deploymentId: source.id, projectId: source.projectId, agentId: source.agentId, commitSha: source.commitSha, source: createSourceIntent({ sourceMode: "image", requestedReference: `registry.example.com/app@sha256:${"a".repeat(64)}` }, { policyVersion: "p1", trustedHosts: ["registry.example.com"], allowTags: false, allowDigests: true }), configRevision: "c1", runtimeRevision: "r1", runtimePort: 3000, secretRefs: [], policyVersion: "p1", schemaVersion: 1 }, { sha256: () => "a".repeat(64) });
       const dispatcher: DeploymentDispatcher = { available: () => true, async dispatch() { return { deploymentId: "wrong", projectId: "wrong", sourceDeploymentId: "wrong", snapshotHash: "b".repeat(64), correlationId: "wrong", effectiveImage: `registry.example.com/app@sha256:${"a".repeat(64)}`, runtimePort: 3000, health: "passed", terminalStatus: "succeeded", rollback: { target: null, result: "not-required" }, proven: true }; } }; const { app } = await authFixture({ state: { deployments, snapshots: { saveSnapshot: async () => undefined, findByHash: async () => snapshot }, deploymentDispatcher: dispatcher, controlGrants: { listForActor: async (actorId: string) => [{ id: "redeploy-grant", actorId, action: "deployment.redeploy" as const, scope: { kind: "platform" as const } }] } } }); const cookie = await loginCookie(app); const url = `/api/v1/deployments/${source.id}/redeploy`; const headers = { cookie, "x-control-idempotency-key": "redeploy-mismatch" }; const pending = await app.inject({ method: "POST", url, headers, payload: { snapshotHash: snapshot.hash } }); const completed = await app.inject({ method: "POST", url, headers: { ...headers, "x-control-confirmation-id": pending.json().data.confirmationId }, payload: { snapshotHash: snapshot.hash } }); expect(completed.statusCode).toBe(502); expect((await deployments.list()).find((item) => item.id !== source.id)?.status).toBe("running");
     });
   });
});
