...
export function buildDockerRestoreInspectArgv(containerName: string): readonly string[] { assertId(containerName, "container name"); return Object.freeze(["docker", "inspect", "--format", "{{index .Config.Labels \"com.deploylite.owner\"}}|{{index .Config.Labels \"com.deploylite.project\"}}|{{index .Config.Labels \"com.deploylite.deployment\"}}|{{index .Config.Labels \"com.deploylite.candidate\"}}|{{index .Config.Labels \"com.deploylite.image\"}}|{{.State.Status}}|{{if .State.Health}}{{.State.Health.Status}}{{end}}", containerName]); }
export function buildDockerStartArgv(containerName: string): readonly string[] { assertId(containerName, "container name"); return Object.freeze(["docker", "start", containerName]); }
