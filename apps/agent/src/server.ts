...
import { agentExecutionReceiptSchema, deploymentStopAgentReceiptSchema } from "@deploylite/contracts";
...
export async function startAgentServer(options: AgentServerOptions) {
  ...
  const server = createServer(async (request: IncomingMessage, response: ServerResponse) => {
    ...
    if (request.method !== "POST" || !["/deployments/execute", "/deployments/stop"].includes(request.url ?? "")) { response.writeHead(404).end(); return; }
